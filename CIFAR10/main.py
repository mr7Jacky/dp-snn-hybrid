...
trainset = tonic.datasets.CIFAR10DVS(save_to='./data', transform=frame_transform, train=True)
trainset= torch.utils.data.Subset(trainset, np.arange(0, len(trainset), 1))
testset = tonic.datasets.CIFAR10DVS(save_to='./data', transform=frame_transform, train=False)
testset = torch.utils.data.Subset(testset, np.arange(0, len(testset), 1))
# create dataloaders
